...
def dicts_to_table(
    ...
    fcodes: list[str] | None = None,
    convert_headers: dict[str, Callable] | None = None,
    ...
    s = "│".join(
        f"{{{h}:{pad}{fcode}}}"
        for h, pad, fcode in zip_longest(marked_keys, pads, fcodes)
    )
    ...
    for d in dicts:
        ...
        if replace_values:
            for k in d:
                ...
                if d[k] is None:
                    raise ValueError(
                        f"bad or no mapping for key '{k}' is None. Use skip or change replace mapping."
                    )
        elif none_keys:
            raise ValueError(
                f"keys {none_keys} are None in {d}. Do skip or use replace mapping."
            )
